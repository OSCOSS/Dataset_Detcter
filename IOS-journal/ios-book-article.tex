...
%<newpart_29
A human expert has finally verified the phrase list, and false positives are added to the %\todo{CL@BG: not sure what “the related list” is; try to phrase more clearly \\ BG: changed it!}
list which contains false positive items.
In the phrase list, there are few false positives, and most can be detected by human expert\todo{CL@BG: not sure why and how they can be detected “while processing articles”; try to clarify \\BG: I add some more sentences to clarify.}while our algorithm processes articles.
Human expert should decide about the output of the algorithm each time. So each time a false output generated by a False phrase occurs, the expert can easily add the phrase to the False positive list. This means our approach will improve over time.
%newpart_29>
...
Although the {\dara} registry is large and it is growing
\todo{CL@BG: provide a figure for \emph{how} fast it is growing BG: I removed "fast" temporarily to solve the problem. @philip: do you have any information about it?}%fast
, there are still many datasets that have not yet been registered there. 
