...
%\todo{SV@BG:say a bit about this standard\\ CL: Yes, please – or, if you can't, shorten this sentence to “see, e.g., \cite{altman2007proposed}” and merge it into the previous one. BG: checked it}
Since the structure of scientific documents does not always follow a standard, and since datasets are rarely being linked to in a consistent ways,
%\todo{CL@BG: Previously, this sentence was phrased poorly up to this point; I tried to improve, but please check whether I preserved your intended meaning. BG: Checked it}
simple keyword or name extraction approaches do not solve the problem \citep{Nadeau2007}. 
Table~\ref{table:citation-variety} shows concrete examples of different reference styles from \todo{CL@BG: say more about these papers.  At the very least what journal they are from.  Even better would be a footnote with citations of all papers.  Journals value precision and completeness. \\ BG: I mentioned the journal in the text and I added DOIs to the table}five papers from the mda journal (see Section~\ref{sec:mda}) that have cited different versions of a study (ALLBUS/GGSS = Allgemeine Bev\"olkerungsumfrage der Sozialwissenschaften/German General Social Survey).%\todo{CL@BG: For Paper C, is the word form “Allgemeinen” really correct? BG: Checked it, it is correct.}
...
The challenge that our research aims at is to turn each dataset reference detected in a paper into an explicit link, for example, using the DOI of the dataset entry in a dataset registry. 
%<newpart_6
%\todo{CL@BG: I rephrased this completely; please check whether I preserved the intended meaning. BG: checked it.}
...
For instance, a dataset may have \emph{set} semantics (e.g. “Set of RDF triples”)~\cite{renear2010definitions} so then it follows the mathematical definition of set\todo{CL@BG: please phrase more clearly; I find “not lose or accept” hard to understand. \\BG@CL: I read the part in the referenced paper and I replaced it with a new sentence. Please check the new one.}, or it may have \emph{collection} semantics so the deletion and addition of data do not have any effect on the dataset's identity.
...
Finally, the \emph{purpose} feature refers to the idea of the \todo{CL@BG: Is this classification only about research data, or could it be more general?  In the latter case, how about rephrasing to “use case” and/or “application domain”? \\ BG:We can say Scientific "activity" (same term used in the reference). }scientific activity which the dataset is created for.
%newpart_7>
...
Based on this model, documents and the query can be displayed in different ways, such as binary vectors, count vectors, and weight vectors\todo{CL@BG: Does your algorithm employ each of these vectors?  If not, it would make sense to shorten this section to just those vector representations that you use. \\BG: The extra parts were removed}.
...
%<newpart_9
%\todo{CL: phrase this sentence in a less redundant way, e.g.\ like “the weight of \dots is \dots”. BG: corrected}
%Weighing the score based on 
...
%newpart_11>
%\todo{CL@BG: this sentence and the following one are partly redundant with new content of Section~\ref{sec:tfidf}; rephrase it in a shorter way by reusing terminology from that section. BG: removed }A document can be considered as a vector (point) in a vector space, each dimension of which corresponds to one term in the document corpus. 
%<newpart_12
...
Similarity measures such as Matching, %\todo{CL@BG: In Section~\ref{sec:cosine} you introduced “Dice”, not “Dice 2”.  Please clarify. BG: Fixed}
...
They employed the normalized google distance %todo BG: normalized google distance? - solved
...
%\todo{CL@BG: really “quality”, or rather “impact”?  (Well, if the cited literature speaks of “quality”, then keep this term here.) BG: fixed}
impact factor of a paper~\todo{CL@BG: The combination of some author/year citations and some numeric citations, like here, looks strange to me.  Please switch everything to \emph{one} citation style: the one preferred by the journal.}\cite{Afzal2010}.
...
%\todo{CL@BG: not fully clear to me what “those” refers to: titles of literature listed in DBLP? BG:solved}
titles found in DBLP.
Our approach tries to match dataset references in a paper to the titles of datasets registered in the {\dara} \todo{CL@BG: consistently use either “registry” or “repository”. \\BG: I have applied the change for dara. I used "registry"}registry.
%newpart_18>
...
%newpart_22>
 ...
 It has published research on all aspects of science of surveys, be it on data collection, measurement, or data analysis and statistics.%\todo{CL@BG/PM: I commented the following sentence as it read too much like an advertisement of mda and didn't add relevant information about our research. BG: it is ok}
...
%newpart_23>
\todo{CL@BG/PM: Once more this is IMHO a bit too much advertising {\dara}.  Can't we simply write “{\dara} assigns a DOI to each dataset”? \\BG: improved}
{\dara} makes 
...
This metadata is exposed to harvesters employing a freely accessible API using OAI-PMH (Open Archives Initiative Protocol for Metadata Harvesting\footnote{\url{http://da-ra.de/oaip/}}). 
  ...
  %\todo{CL@BG: of \emph{datasets}  , or actually of dataset \emph{references}? BG: fixed}
...
\label{sec:eval}
 \label{sec:eval}
The calculation of evaluation metrics such as precision, recall, and F-measure require a ground truth. We therefore selected a test corpus of 15 random papers from the 2013 and 2014 issues of the mda journal (see Section~\ref{sec:mda}) -- 6 in English and 9 in German. %todo: link to the mda dataset -solved
%<newpart_36
