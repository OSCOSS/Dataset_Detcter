...
+\address[C]{Intelligent Analysis and Information Systems (IAIS), Fraunhofer}
\begin{abstract}
\todo[inline, size=\tiny]{SV@BG: usually negative start is not the best way to start, I suggest that you turn the sentence in a positive one and add one introductory sentence also, you are too quick to say what is there and what we lack. I did some changes already.}
Scientific full text papers are usually stored in separate places than their underlying research datasets.
%<newpart_0
Although there exist many digital registries that make scientific datasets citable, authoring tools lack the integration to those repositories. 
%newpart_0>
As a consequence, explicit links to datasets are generally missing while authors refer to datasets using \emph{titles} and \emph{year} of publication. 
Manually detecting references to datasets in scientific papers is a time-consuming task and requires an expert in the underlying research domain. 
In this research work, we propose and evaluate a semi-automatic approach for finding explicit references to datasets in social sciences papers.

%<newpart_1
Our approach includes three main steps: extracting pre-defined special features from datasets' titles in {\dara} registry, detecting datasets' references using the extracted features, and matching the found references with corresponding datasets' titles.
%newpart_1> 
The approach does not need a corpus of papers (no cold start problem)\todo[inline, size=\tiny]{SV@BG:I do not understand this} and it performs well on a small test corpus (gold standard). 
Our approach achieved an F-measure of 0.84 for detecting references in full texts and an F-measure of 0.83 for finding correct matches of detected references in the {\dara} dataset registry. \todo[inline, size=\tiny]{SV@BG:I would not give exact figures here, tell a general results, what we have achieved}
...
Digital libraries have been growing enormously in recent years. \todo[inline, size=\tiny]{SV@BG:try to have sentences that carry some value, this is not good to start the intro with such sentence, align it with abstract, even use the same sentence and expand each sentence of the abstract as one paragraph in intro}
Digital libraries provide resources with high metadata quality, easy subject access, and support for retrieving information~\citep{Hienert2015}. 
We are specifically interested in scientific full text papers in digital libraries.\todo[inline, size=\tiny]{SV@BG:saying we are interested on sth is not scientific at all, rephrase it and include the reason}
...
Explicit links from scientific publications to the underlying datasets and vice versa can be useful in multiple use cases as follows:
\begin{itemize}
	\item reviewers exploring to check the evaluation mentioned in a paper which is performed on a dataset, 
	%a link would give him straightforward access to the data, enabling them to check the evaluation,
	\item researchers desiring to perform further analysis on a dataset that was used in a paper.
\end{itemize} \todo[inline, size=\tiny]{SV@BG:add two more use cases} 

Currently, majority of already published papers lack such direct links to datasets.
\todo[inline, size=\tiny]{SV@BG:add one more sentence about the state of current papers, how they do refer to datasets}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=3.5in]{DistPlaces3.PNG} \todo[inline, size=\tiny]{SV@BG:bad quality, recheck the figure}
	\caption{The distribution of dataset references in 15 random mda\todo[inline, size=\tiny]{SV@BG:never said before what mda is} papers}
	\label{fig:places-example}
\end{figure}

%<newpart_3
This variance even makes rule-based approaches difficult, as it is hard to cover all cases. \todo[inline, size=\tiny]{SV@BG:give 1-2 concrete technical reasons why it makes it difficult}
...
In the remainder of this section... \todo[inline, size=\tiny]{SV@BG:say what is going to come in subsections} 

\subsection{Problem Statement and Contributions}
Whereas a lot of effort has been done on information extraction in general~\citep{Sarawagi2007}, fewer attempts have focused on the specific task of dataset extraction (see e.g.~\citep{MeiyuLu2012}). \todo[inline, size=\tiny]{SV@BG:effort on information extraction and dataset extraction is not comparable, one is a reasearch field, the other is a usecase, rephrase it.}
...
\todo[inline, size=\tiny]{SV@BG:say a bit about this standard}
