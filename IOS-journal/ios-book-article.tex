...
%newpart_0>
However, \todo[inline]{CL@BG: This statement actually creates the obligation for us to say something about the possible integration of our implementation into authoring tools in “future work”.  At least 2–3 sentences.  Think of open, pluggable systems, think of Fidus Writer.}as they are not integrated with authoring tools, explicit links to datasets are generally missing in papers and authors typically refer to datasets using not standardized titles and often year of publication.
\todo[inline]{CL: The following is a new and IMHO important point.\\ BG@CL: This is a great point but we should not exceed more than 200 words.- now the abstract contains 220 words.\\ CL: I shortened it to 210 for now; maybe they will either accept 210, or someone else can shorten a bit further ;-)}It is thus hard to reproduce research results, to access datasets for further analysis, and to determine the impact of a dataset.
...
%<newpart_1
We first extract pre-defined special features from datasets' titles in the {\dara} registry, then detect references to datasets using the extracted features, and finally match the references found with corresponding datasets' titles.
%newpart_1> 
The approach does not require a corpus of papers (no cold start problem) and performs well on a small test corpus. 
