...
Users should decide about the output of the algorithm each time. So each time a false output generated by a False phrase occurs, the users can easily add the phrase to the False positive list. This means our approach will improve over time.
%newpart_29>
...
\todo{CL@BG: I have difficulties understanding the following sentence. BG: I added more sentences to make it more clear.}The identification of central dataset(s) is possible after pairing a share of references of datasets in a given article with titles in {\dara}, and then this identification affects the ranking of the rest of the references. A central dataset in a paper is referred to a dataset which the paper is written based on that and usually, they mention in the paper many times.
